...
the triage outcome *straight to `main`* already says it. The `pre-approved`
milestone is set on projects.
...
Two things make this concrete rather than a pious line:

- **0.7.2 exited `1` with every success line already printed.** The status said
  *the deploy failed*; production was live and correct. Nothing was missing from
  the code — what was missing was anyone having asked what that exit would mean
  to a reader. #319, and D41 is the answer
- **`announce_release_checks` reports a moment rather than a state**, so when it
  was skipped there was nothing to go back to. #310's R2 is still outstanding
  because of it
...
one the script means or one it did not expect. **An undocumented error is an
...
`/auth/login` returns *"Incorrect name or password"* for **two different
conditions**, deliberately, so a caller cannot enumerate display names — and it
verifies a decoy password on the miss so both paths cost the same. None of that
is visible in a request type, a response type or a status code, and all of it is
what a caller needs.
...
| what a script reports or logs, and what it exits with | [`3.0`](3.0-tools.md) for the script, [`4.8`](4.8-artefacts.md) for what an exit status means |
...
| **`Open actions`** | what the decision creates, `(Steve)` or `(Claude)` prefixed so `actions.py` shows it |
...
**`Feedback Provided` is named for the outstanding act**, as the others are. Two
words were rejected and the reasons are worth keeping: `Proposed` misleads,
because an `Asked` decision already carries a proposal; and **`Answered` is
taken** — 37 glossary decisions are marked *answered*, and D36 defines it as
*settled and due to be applied*, which is `Decided`.
