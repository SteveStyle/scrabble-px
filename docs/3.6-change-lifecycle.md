...
self-contained, it has mechanisms of its own — framing, broadcast, reconnection,
ordering, the version pair — and every function uses it. It is not one. Owner,
2026-08-28: *"we need a workstream to own the general messaging mechanism, which
is shared between the functions. Game defines most of the actual messages and
data types, so it best fits there."*

| | owner |
| --- | --- |
| the **mechanism**, shared by everything | the workstream that defines most of what flows through it |
| the **messages** each function sends through it | that function's own workstream |
...
**In practice**: a change to *how* anything is delivered belongs to the mechanism's
owner, whoever it is delivered for. A change to *what* is said belongs to whatever
is saying it.
...
| **Candidate Project** | add to an existing project; a group of requirements create a project; a single requirement project |

**The last one is three outcomes, not one**, and that is deliberate: a candidate
may join a project that already exists, combine with others into a new one, or
be a project on its own. Which of the three it turns out to be is a planning
decision, and planning is where it belongs — triage gathers, planning decides.
