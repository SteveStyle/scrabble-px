...
**The one honest exception**, and it is why the older reading looked right: a
script is live for *its author* the moment it is committed locally, because
scripts run from the working tree. That is delivery to an audience of one. The
practice of pushing immediately after committing is what keeps the two from
diverging, and is the reason the distinction is invisible in normal use.
