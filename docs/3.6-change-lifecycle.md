...
> **A branch exists to hold a change back.** Branch when you want to keep the
> change out of `main` for a while — because a project is developing it, or
> because it is a significant change to an existing document and wants review.
> Commit straight to `main` when you are happy for the change to go live
> immediately. **A change that needs no branch is a pre-approved change**; the
> two descriptions are the same test asked from opposite ends.
...
It is worth preferring over a list of cases because it asks about **intent**
rather than about a change's size or kind. A large change you want live now
belongs on `main`; a one-line change you want looked at first does not.
