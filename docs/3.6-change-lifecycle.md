...
**The levels, because every rule below is stated at one of them.**
...
**Workstreams are in [3.7](3.7-workstreams.md)**, which owns them: the ten
capabilities, what each owns, and where the boundaries fall. A workstream's name
is a proper noun and is capitalised.
...
**Fast-forward if you can, and you usually can.** A release branch that has
...
If a fast-forward is not possible, `main` has moved and the release branch owes
...
The way to avoid paying it is to keep the release branch current while the
...
**A release branch does not**, because it never becomes a pull request — it is
...
feature branch that has gone into a release branch is done, months before that
...
against having branches for documents, it keeps going wrong."* And it did, three
times in three days — `issue-71-detailed-design` sat five days while `main` moved
57 commits ahead and its pull request was abandoned rather than merged; two
document commits landed on branches they did not belong to because a branch was
still checked out.

**The failure is structural, not carelessness.** A document branch is never
urgent, so it lives a long time; `main` moves; the diff stops being reviewable;
and the review the branch existed to provide is the thing that does not happen.

**Where a project has a branch, its documents go on it** — the one-place rule
outranks this, and no issue has changes in `main` and changes in a branch. What
has gone is the idea that a documentation change should take a branch *in order
to be reviewed*. The review is the discussion, and the diff on `main` is there to
read afterwards.
...
#### What a pull request is for here
...
merging is a separate act, made locally with a written message rather than by a
...
`e2e` runs on pull requests and on `main`. So opening one is how a branch gets
the full suite.
