...
**A delivery that is not a release and is not the last one gets no issue.** Such
deliveries are generally independent of other changes, so they need no planning
of that kind. The delivery log records them, and the project's deliveries table
plans them.
...
| an earlier non-release | previous semver plus a letter | none |
...
**The runbook lives with the delivery.** Where a delivery has its own issue, its
steps live there and the project's table links to it. Otherwise the steps are in
the table. Never both.
...
on all three issue types.

**Unset is not the same as `Not Pre-approved`.** An unset field is invisible to a
board filter and to a query, so an issue nobody has decided about does not appear
in either list — which is the trap the board has already sprung twice. Absence
means *nobody has said*, and it is the state to look for, not a safe default.
