...
`.github/ISSUE_TEMPLATE/decision.yml`. It has no `Stage` and no `Phase`: open
and closed carry the whole state, which is the point.
...
| **closed** | **means applied.** There is an agreed decision and every action is ticked — checked by `check-transitions.sh`, which `verify.sh` reports as a note |
