...
**So park the project issue in the milestone of its last delivery.** Earlier
releases then ignore it, and the final one closes it. No tooling change, no
label, no sub-issue graph to read.
...
**Still unsolved**: a project deliberately outside the milestones looks exactly
like one nobody has triaged. See #195.
...
**A project issue carries six headings**, each holding either text or a link to
...
bodies have opposite lifecycles. A requirement issue's body is what somebody
noticed when they noticed it, and is **never edited**; a project issue's body is
six headings kept current, and a stale one is worse than none. Overwriting the
...
**Requirements → triage → triaged requirements → project planning →
projects.** Owner, 2026-08-22, arriving at it in two steps — the three-stage form
first, then the two that were hiding inside the middle of it.

| | | what it is |
| --- | --- | --- |
| **requirements** | *a thing* | something should be true and is not. The **Requirement** form: five seconds, and **never edited afterwards** |
| **triage** | *an act* | per requirement, and cheap: do we want it, what kind is it, what would it touch, whose capability |
| **triaged requirements** | *a state* | classified and filed, not yet scheduled or grouped. **This is what the backlog is** — most of forty-five open issues |
| **project planning** | *an act* | across requirements, and it waits for appetite: what do we do next, and what goes together |
| **projects** | *a thing* | where work happens. The **Project** form: six headings, kept current |

#### How to say it: three states of the work, and one activity that never stops

The five read as a list of five nouns, and they are not. **Three are states and
two are activities** — which is what a process is, a state machine — and one of
the two activities does not sit on the line at all.

```text
requirement ──triage──▶ requirement, triaged ──onboarding──▶ project ──delivery──▶ live
              enriches  └──── the backlog ────┘   creates

                    project planning  ─ across every open project, continuously ─
```

**Triage enriches; onboarding creates.** Owner, 2026-08-22: *"the requirement
issue is enriched by triage and impact assessment, not replaced."* Triage adds
labels and a comment carrying the artefact guess; **the body is untouched**, so
the record of what was noticed survives. Onboarding is the transition that makes
a new thing, and folds the requirement into it.

**Triaged is a working state, not a finished one — D35.** Owner, 2026-08-24:
...
is in the state triaged it may go through many edits, because it hasn't yet been
closed its future still needs to be considered and decided."*

| state | what it means | open? |
| --- | --- | --- |
| **untriaged** | nobody has looked at it | yes |
| **in triage** | being assessed. It may be edited many times, and **its future is still undecided** | yes |
| **planned** | folded into a project, and **closed** | no |

**So the backlog is the two open states**, and *planned* is not a third bucket of
open issues — it is what leaving the backlog looks like. An open requirement
sitting under a project is therefore an anomaly rather than a state: the fold
should have closed it (D33, D18).
...
**Prefer *in triage* to *triaged*** when naming the state. The past participle
reads as *finished being triaged*, which is the opposite of what it means.

**Project planning is not a stage the work passes through.** It runs across every
open project, continuously, and it is where dependencies are understood and order
is decided.

#### A decision is applied when it is made — D36
...
| 8 | **What do we know now that the raiser did not?** | the comment. **The body is never edited** |
...
**A measure, because *be brief* is not enforceable.** The Integration Domain CIP,
which this process resembles closely, carried 143 initiatives with a median
description of **197 characters** and a median benefit statement of **55**. The
first three triage comments written here ran to **2,240**, and rewriting them
lost no finding at all — only the reasoning behind each, which is what a design
is for.
...
different process takes over. Owner, 2026-08-28, on the Integration Domain CIP he
ran before this: *"priority should stay. But it stops when the requirement is
folded into a project and a new process takes over. That was the same with CIP.
Once a requirement was passed into a delivery vehicle the CIP process stopped,
apart from noting when it was done."*
...
**A limitation worth stating rather than discovering.** The CIP scored each
initiative against fourteen named objectives and totalled them, so *breadth* was
derived and two people scoring the same item agreed. Those fourteen were
**contractual SLAs** — they existed because somebody was obliged to meet them.
This project has no equivalent, and no measure of how well it does any of it. So
`Priority` here is a judgement and will stay one, which is a known limit on it
...
Owner, 2026-08-22: *"when I wrote an impact assessment for a new IRIS request I
