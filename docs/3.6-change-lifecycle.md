...
| a **Delivery** sub-issue, one per earlier delivery | the release that shipped it | closed by that release; it is one unit of work and it is done |

A project with a single delivery needs no sub-issue, which is most of them.
Only a project that ships more than once pays for the extra issues, and it pays
one per earlier delivery.
...
part of it did, and earlier deliveries left no record of which release carried
them. #240 is the worked example: delivery 1 redeployed rehearsal, delivery 2
rides 0.7.1, and before this rule only the second was visible anywhere.
