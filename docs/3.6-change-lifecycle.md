...
| **`Decision State`** | `Asked · For Agreement · Ready for Sign Off · Decided · Actioned`, the columns of the Decisions board. It says who is holding it |
...
| `For Agreement` | **Claude** | he has commented; Claude responds and **writes the decision down** |
| `Ready for Sign Off` | **the owner** | it is documented; he reads it, ticks the box and moves it on |
...
**Documenting it is what ends `For Agreement`** — not Claude agreeing. That puts
the *Agreed Decision* section on the critical path rather than beside it, and
gives the owner something to sign off against.

**`Ready for Sign Off` is not redundant with `Decided`.** I argued it was, and
...
**`For Agreement` is named for the outstanding act**, as the others are. Two
