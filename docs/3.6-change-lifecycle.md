...
#### How to say it: two states of one thing, then a second thing

The five read as a list of five nouns, and they are not. **Three are states or
things and two are activities**, which is what a process is — a state machine —
so the flow is clearer said as states with named transitions between them:

```text
requirement ──triage──▶ requirement, triaged ──project planning──▶ project ──delivery──▶ live
                        └──── the backlog ────┘
```
