...
| 2 | **What kind of change is it?** | the type label |
...
| 6 | **How big is it, roughly?** | the comment. A band, not an estimate |
...
| **workstream** | game lifecycle · engine player · client UI · client management · auth · capacity planning · process definition · delivery tooling · operations |
| **size** | small · medium · large · unknown — a band, not an estimate |
...
can give yet is written down as *unknown*, and nothing downstream is waiting on a
number. Triage may **recommend** a sequence; it does not set one.
