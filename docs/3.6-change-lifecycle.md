...
| **`Decision State`** | `Asked · For agreement · Decided · Actioned`, the columns of the Decisions board. It says whose turn it is |
...
| state | waiting for |
| --- | --- |
| `Asked` | the owner to choose. Claude raised it with options and a recommendation |
| **`For agreement`** | both have spoken and a candidate answer exists. Either may confirm it into `Decided`, or put it back to `Asked` for another round |
| `Decided` | agreed, not yet applied |
| `Actioned` | applied, not yet read |
