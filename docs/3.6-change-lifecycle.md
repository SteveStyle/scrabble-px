...
**Requirements → triage → projects.** Owner, 2026-08-22. Three stages with a
different kind of issue at each end, which is what makes the middle one a
decision rather than a formality:

| | | |
| --- | --- | --- |
| **requirements** | something should be true and is not | the **Requirement** form. Five seconds, never edited afterwards |
| **triage** | do we want it, what kind is it, what would it touch, whose capability, and does it join work or start it | no artefact of its own — it *produces* one |
| **projects** | where the work happens | the **Project** form. Five headings, kept current for as long as the project runs |

The two forms exist because the two bodies have opposite lifecycles — see
[2.11](#211-what-a-project-is-and-what-its-issue-carries). Triage is where one
becomes the other, and **it never does so by editing**: it folds.

**This is the first three of the five stages** the industry names — demand
management, impact assessment, project onboarding — with release management
following in [Part 1](#1-the-flow-how-a-change-reaches-production). Pipeline
management is the thin one, and still is.
