...
**A milestone is not a queue.** It was one, and `Phase` took that job: `Q1`,
`Q2` and `Q3` say what is next off the blocks. What is left to the milestone is
one of two things, depending on whether the release has been chosen yet — owner,
2026-09-04: *"it is either naming the delivery label, or saying at what level the
semver should change, as a minimum."*

An issue carries exactly one:
...
**`patch`, `minor` and `major` were empty on 2026-09-04** and are kept anyway,
deliberately: they are the minimum-semver statement for work whose release is
not yet chosen, and nothing has needed to say that since `Phase` took over the
queueing. Owner: *"maybe we don't need them, but let's see if they are useful."*
Recorded so the next reader finds a decision rather than three unused values.
...
**Documentation and tooling projects are marked `Pre-approved`.** They go
straight to `main`, so they make no delivery: no milestone, no row in the log.
...
pre-approved. **Leaving the field unset is how the question gets asked.**
...
| #283, #310, #299 | straight to `main`, pre-approved | no milestone, no row |
...
the triage outcome *straight to `main`* already says what `Pre-approved` would.
That field is set on projects.
...
| `no-release` | **no milestone, `Pre-approved` set** | it goes straight to `main` |
...
**Rejected: folding `Pre-approved` into the milestone as a value.** It is
tempting — the two are mutually exclusive, nothing in the tooling reads the
field, and the milestone already holds non-release values. It fails on what
the milestone field *means*: which delivery carries this change. A pre-approved
project makes no delivery, so `pre-approved` as a milestone would be a delivery
label saying there is no delivery. Two questions, two fields.
...
**A delivery's steps live in one place.** An earlier delivery that has its own
`Project Delivery` issue keeps its runbook there, and the project's deliveries
table links to it. The last delivery has no issue of its own, so its runbook
sits under that table. Never both: a runbook written twice is two places to
update, and the one nobody is looking at is the one that goes stale (#283).
...
**The roadmap is which projects go together, and in what order.** It lives in
[1.5](1.5-work-in-progress.md), drawn from the issues by
`scripts/roadmap-diagram.py` and regenerated rather than maintained.
