...
sit beside field values like `prod-tooling` and `Scope and design`, where the
...
#### A project issue goes in its LAST delivery's milestone

`deploy.sh` closes every open issue in the milestone it is shipping. That is
right for an issue that is one unit of work — it is what makes a milestone a
shipping list. It is wrong for a **project**, which may have more than one
delivery, and it closed #174 on 2026-08-22 when one of its three had shipped.

**So park the project issue in the milestone of its last delivery**, and give
each earlier delivery its own issue, typed **Delivery**, carrying the milestone
that shipped it.

Owner, 2026-09-01: *"the project carries the milestone of the final delivery.
Earlier deliveries are separate sub-issues with a new Issue Type."*

| | carries the milestone of | |
| --- | --- | --- |
| the project issue | its final delivery | closed or advanced by that release |
| a **Delivery** sub-issue, one per earlier delivery **that a release carried** | the release that shipped it | closed by that release; it is one unit of work and it is done |

**Only deliveries an application release carried get an issue.** Owner,
2026-09-01. A milestone names a release, so it can only label a delivery a
release carried. A merge is live when it lands, a host change when it is typed,
a rehearsal deploy consumes no version — none of them has a release to be
labelled with, and giving them one would put a number on something that number
does not describe.

**A non-release delivery is recorded in the delivery log, and gets no issue.**
Owner, 2026-09-01: *"they are project deliveries and should be recorded in the
delivery log. However they don't need a Project Delivery issue creating for
them, because only Releases need careful planning."* An issue is a place to
plan; a log row is a place to record. A delivery that ships by merge, by hand,
or to a non-production environment is planned in the project's deliveries table
and recorded in [4.9](4.9-delivery-log.md) under a lettered identifier. Only a
release earns an issue of its own, and only then when it is not the project's
final delivery.

**And the lettered identifier does not need a milestone behind it.** A milestone
exists so the project issue has somewhere to sit, so a letter milestone is
created only when the non-release delivery is the project's **last or only**
one. An earlier one needs none: the log row carries the identifier, and the
project sits in its final delivery's milestone. Owner, 2026-09-01, on a letter
milestone created for #240's first delivery: *"it doesn't do any harm... but it
isn't necessary. I am trying to reduce the amount of admin."*

**This is about the label, not about the worth.** A delivery to a
non-production environment is a delivery in full: it reaches its users and it
delivers a benefit. Owner, 2026-09-01, on #240: *"the first delivers the benefit
of controlling access to Rehearsal. The second delivers the benefit of keeping
Rehearsal and Production in line."* Two deliveries of one change to two
environments, each with its own benefit, and only the second is a release.

**So the same change delivered twice is two deliveries, not a delivery and a
check on it.** Each is named, each states what it is for, and the
post-deployment check answers each benefit separately. Calling the second one a
check would lose the reason it exists — here, that one `Caddyfile` serves both
environments, and a file live in one and merged in the other is exactly the
drift that stops rehearsal predicting production.

Everything else stays a **row in the project's deliveries table**, which is
where a delivery is recorded by default. The sub-issue is the exception, not
the rule.

So a project needs a sub-issue only when it **reaches production more than
once**, which is uncommon: #174 with its three deliveries is the shape.
Everything single-delivery, and everything whose earlier deliveries were merges,
carries one milestone on the project and nothing else.

**The runbook moves with the delivery.** An earlier delivery's steps live on its
own issue; the project's deliveries table keeps the final delivery's runbook and
links to the rest. Otherwise the same delivery is written in two places and they
drift.

**What this fixes.** A milestone said a project shipped in a release when only
part of it did, and an earlier delivery that a release carried left no record of
which release that was.

**#240 shows where the line falls.** Delivery 1 rebuilt the image with the new
`Caddyfile` and redeployed rehearsal; delivery 2 rides 0.7.1 to production. Only
delivery 2 is carried by a release, so #240 takes the 0.7.1 milestone and needs
no sub-issue: delivery 1 stays a row. A rehearsal deploy changes the image and consumes no
version, which is why the test is *did a release carry it*, not *did it change
the image*.

**A project whose final delivery is not a release still has no milestone**, and
is still closed by hand. That is the row below, unchanged. It no longer looks
untriaged, because the Phase field says where it is.

**Unless the last delivery is not a release**, in which case no milestone can
close it at all and it is closed by hand — which is the case for every project
ending in a host change or a drill.

| the project's last delivery | what closes the project issue |
| --- | --- |
| **is a release** | the milestone it is parked in |
| **is not a release** | nobody. Close it by hand when the delivery is applied |
...
([2.15](#215-classifying-a-change-type-release-route)).

**`no-release` is a fourth queue, and the odd one out.** Documentation and
...
`fasttrack` — the milestone renamed to `patch` on 2026-08-24 — was wrong in a
way that mattered: it implied they were waiting for
...
`no-release` issue except somebody noticing it is done — which is how the game
lifecycle tests sat finished and open for a fortnight. **A `no-release` issue has to
...
`patch`, a minor from `minor`, a major from `major`. One slot per queue is
...
you. A patch change arriving after the scope is fixed goes to the *next*
...
complex enough to deserve more time than a patch implies.
...
| **post-deployment checks** | one check per requirement, answered after the deploy |
...
**This is the standard-change criteria** that [2.15](#215-classifying-a-change-type-release-route)
...
literal `major-function` and `minor-function`; rename either and every patch
release passes its gate while nothing appears to break. That is the same failure
...
the same slot as `minor-function` and `major-function` and collapses the
distinction between them, which is the one that decides whether a design note
is owed. GitHub's default labels went with the rest on 2026-08-27 —
...
(#234, nothing reports a vulnerable dependency), `prod-tooling` (#235, no ruleset
on `main`) and `non-prod-tooling` (#236, actions never updated). Split on
...
**None of the three says who owns it.** `prod-tooling` and `non-prod-tooling`
look like they divide ownership between workstreams, and they must not be read
that way: **they say how much risk a change carries** and therefore what it owes
— a design, a test approach, a rehearsal. Both appear under Delivery Tooling and
under Operations & Infrastructure, because both of those cover every
environment: preview and rehearsal as well as production, kept deliberately
alike.

So #40 (rehearsal holding production user data on a public IP) is operations
work, despite being a non-production environment. What owns a change is the
workstream, and that is a different question from all three above.
...
Owner, 2026-08-24: *"A script is visible as soon as it is on local `main`, but
the push to origin should always be done at the same time as the commit."*

That is where the artefact reaches its users, and #188's own policy says why:
**documents are read from `origin/main`; scripts are run from the local `main`
in dev; the delivery is updating `main` and pushing.** Until the push, a change
exists for one person on one machine.

*This replaced "one route, two moments" on 2026-08-24.* That reading had a
repository artefact delivered **on save** when edited on `main` and **on merge**
when it came from a branch — but those are two ways the commit **arrives on
`main`**, not two moments of delivery. Whether it arrived by editing or by
merging,
[2.12](#212-a-branch-belongs-to-a-project) already decides which, and nothing is
delivered by either until it is pushed.

**There is no exception for the author's own working tree**, which is what the
older reading mistook for one. Owner, 2026-08-24: *"A script shouldn't be run
from the working tree until it is pushed. Technically you can, but it hasn't
officially been delivered. Suppose the environment was rebuilt on another
computer — only files pushed to origin would be available."*

**That is the test: what would survive a rebuild elsewhere is what has been
delivered.** An unpushed commit exists on one disk.

**Run it to test it, not to use it.** Owner, 2026-08-24: *"you might run it as a
test, but not run it for its intended purpose."* Exercising an unpushed script to
find out whether it works is how it gets built. Using one to do a job — deploy
something, check something, decide something — is depending on a state that
exists nowhere but here, and it will not be reproducible by the person who has
to look at it afterwards.

That is why pushing immediately after committing matters more than it looks: it
is not tidiness, it is the act that makes the change exist.
...
| lane | is now |
| --- | --- |
| `no-release` | route *never leaves the repository*, release **nothing** |
| `patch` | release **patch alone** |
| `minor` | release **minor** |
| `major` | release **major** |
...
| --- | --- | --- |
| **pre-approved** | meets the [standard-change criteria](#22-when-to-branch-and-what-pre-approved-means) | nothing beyond the commit. Straight to `main` |
...
|||
| --- | --- |
| **requirement**  | A request for a change.  Can be raised quickly and analysed in more detail later. |
...
| **scope** | Functional options to meet the requirement.  Technical options.  Identify dependencies.  Make recommendations and set effort accordingly.  |
...
**The classification table is gone, 2026-08-28.** It listed valid values for
`type`, `release` and `effort` and had you delete all but one — because, as the
owner put it on 2026-08-24, *"there wasn't a drop down of valid values… a comment
is free text, so a script meets `prod-tooling`, `prod tooling` and `production
tooling` and has to guess they are the same."*

**There is a dropdown now.** `Type of change`, `Effort` and `Workstream` are
issue fields in the sidebar, with fixed values and no typing. Asking for them
again in a comment records the same fact twice, in the place nothing reads.

**And `release` had nowhere left to go.** Since #229 a milestone belongs to a
project delivery; a requirement carries none. The question *what carries this to
users* is answered when it folds into a project, by whichever project takes it —
which is the only moment anybody can answer it honestly.

**What remains is what no field can hold**: the artefacts, the context, and what
triage found that the raiser did not know. Those were always the point — the
three rows above were bookkeeping that a sidebar now does better.

**No date.** GitHub timestamps every comment beside the author, so one typed
into the heading is redundant — and a saved reply cannot substitute today's date,
so it would be a literal that somebody eventually forgets to change. A comment
carrying the wrong date is worse than one carrying none.

**Question 1 has no row**, because its answer is not a field: if the requirement
is not real or no longer relevant, the issue is **closed** rather than annotated.
*Reject* is an outcome of triage, not a value in it.

**The empty sections are the useful ones.** *Context* and *what we know now* are
where triage earns its place, and a triage that leaves both blank has classified
an issue rather than assessed one.

**And they are short.** Owner, 2026-08-28, reading the first three written under
this model: *"the triage comment should be brief and to the point. The project
will define everything in detail."* Triage is deciding **whether and with what**,
not **how**. A comment that argues the design has done the project's work before
the project exists — and done it without the project's review, its test approach
or its artefact list.

**A measure, because *be brief* is not enforceable.** The first three triage
comments written under this process ran to **2,240 characters** each. Rewriting
two of them to a third of that lost **no finding at all** — only the reasoning
behind each, which is what a design is for.

**The test is not a character count but what a sentence is doing.** State the
finding; do not argue it. *"Still 200 from anywhere, twenty-five days on"* is
triage. *"Of the four options, the first is cheapest because…"* is design, and
saying it here means saying it unreviewed.

**What triage sets**, all in the sidebar: `Workstream`, `Type of change`,
`Effort`, `Priority` and `Stage`. Priority joined that list on 2026-08-28: it had
been left to planning on the argument that priority is about what enters a
release, but the person who has just read the issue is the one who knows how much
it matters, and asking them again later wastes the reading.
...
contains.** Only patch changes in it, and it is a patch; one minor-function
