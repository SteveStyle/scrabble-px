...
**Three categories, and the entry names which one it is:**

| | the claim | where it lives | when it runs |
| --- | --- | --- | --- |
| **not relevant after the project** | was true once — *"the migration converted all 412 rows"* | **the project's folder** under `docs/changes/…`, not `scripts/` | during the project, and never again |
| **part of regression testing** | must hold for as long as the service runs, and can be checked automatically | wherever the suite lives — `crates/*/tests/`, `e2e/` | **every time**, without anybody deciding to |
| **optional regression test** | must hold, but cannot be checked automatically — it needs a running stack, or real hardware, or it is slow | `scripts/`, and listed in [3.0 Tools](3.0-tools.md) | on a stated trigger, and on demand |
...
**And the third category is where tests go to be forgotten**, so it carries one
more obligation: **say why it is optional.** *"It needs a running stack"* and
*"it takes eleven minutes"* are reasons that can stop being true — and when the
reason goes, the test should be promoted to the second category rather than
staying optional out of habit. Without the reason written down, nobody knows
whether it was a judgement or an accident.
