...
| **operations** (#189) | the service once it is live — the host, monitoring, alerting, logging, backups, disaster recovery | never; it is a running concern |
...
**#203 is the index**, pinned, listing the three. Everything below them is
reachable from one without knowing a number.
...
**`needs-triage` is the queue, not the absence of a type.** Added 2026-08-27,
when the first issue raised under this process — #228 — arrived carrying
`Type of change` and `Effort` set at the form, and so was invisible to a triage
view that asked *"has nobody said what kind of change this is"*. Classifying is
not assessing: questions 6 to 8 are the ones triage exists for, and an issue can
answer the first five at the moment it is raised.

**So every route that raises an issue applies the label.** The requirement form
does it by declaration; `gh issue create` needs `--label needs-triage` passed. A
route that forgets does not raise an error — the issue simply never appears in
the queue, which is the failure mode this whole section is about.

**Folding removes `needs-triage`.** Owner, 2026-08-25, noticing #216 closed and
folded while still marked untriaged: *"should this be removed automatically by
folding?"* It should, because **folding is triage** — you cannot know which
project takes a requirement without having assessed it, so the two labels
contradict each other. Five folded issues carried it before this was noticed.
...
An issue becomes two when it **touches both tooling and the app**, when it
**will be two releases**, or when it **spans two workstreams**. The second is the
one that gets missed: two releases means two changes even when the type is
identical.
...
under operations, because both of those cover every environment: preview and
rehearsal as well as production, kept deliberately alike.
...
| | *delete all but one* |
| --- | --- |
| **type** | bug · minor-function · major-function · appearance · documentation · non-prod-tooling · prod-tooling |
| **release** | not decided yet · no-release · patch · minor · major |
| **effort** | high · medium · low — a band, not an estimate. Delete the row if triage cannot say |
...
**Every cell lists its valid values, and you delete all but one.** Owner,
2026-08-24, filling it in for the first time: *"it was a bit painful because
there wasn't a drop down of valid values, or even a list in most cases. At
minimum there should be a list. That would also help the tooling as otherwise
values entered will vary."*

The second half is the stronger reason. A comment is free text, so without the
list a reader — or a script — meets `prod-tooling`, `prod tooling` and
`production tooling` and has to guess they are the same. A GitHub comment box
cannot offer a dropdown, so listing the values **is** the constraint.
...
Adopted rather than coined — `scripts/roadmap.sh` already prints exactly this,
*"what is in each release, in release order, and what waits on what"*.
...
change when it resolves. `roadmap.sh` reads `#N` references from issue bodies
today and calls the column *mentioned by* rather than *depends on*, *"evidence to
read rather than a graph to trust"* — a structured heading is what would let it
...
machinery to hold it: a type label present, `needs-triage` absent, not yet
folded. *Triaged requirements* is not a fourth noun; it is an adjective doing
honest work.
...
A **triaged requirement** is an open issue with a type label, without
`needs-triage`, and not yet folded into a project. All three are already true or
queryable, so the state is visible without inventing anything to hold it.
...
The tools for it exist — `status.sh`, `roadmap.sh`, and an overlap between two
artefact lists. What is still missing is the **rule**: when the pipeline is
