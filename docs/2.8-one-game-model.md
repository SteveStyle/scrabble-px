...
An engine still gets what it needs — the board, its own rack, the rules, all
visible to its own seat. What it must not get is a confident wrong answer
...
Open: whether the proxy learns of a turn by subscribing or by polling, and
whether a bot has an account of its own. Both are argued below.
