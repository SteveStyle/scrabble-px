...
Worth knowing rather than fixing. A client that waits for each response before
sending the next has no such window, which is what a well-behaved one does
anyway.
