...
Everything seat-scoped follows from the same change — which racks are visible,
which turns are yours, which messages are addressed to you. **Turn
notification names the seat, not the account**, or "it is your turn" is
ambiguous the moment anybody holds two.
...
Open: bot accounts need credentials, and a session lasts ten days at most and
ends after forty-eight hours unused (ACC-1). A long-running proxy has to log
in again as a matter of course, or bot accounts need an exemption. Worth
deciding rather than discovering when the bots go quiet.
