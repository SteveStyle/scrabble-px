...
sequenceDiagram
    ...
    Note over Dev,Prod: 1.6 — deploy.sh: eight gates, then ship
...
**Which steps a change needs depends on its `Route`** — see
[3.6 §2.15](3.6-change-lifecycle.md#215-classifying-a-change).

| route | steps |
| --- | --- |
| **Production Release** — anything built into the image | 1.1 – 1.7 |
| **Repository Change** — anything else in the repository | 1.1, 1.2, then push. Live on `origin/main`, so no 1.3–1.7 |
...
cases are a release taking changes from multiple projects, and a large project with
multiple work packages.

Changes are only made on the development branches, never on the release branch directly, so the
release branch can always be rebuilt from the source branches.  The scope can be changed by
rebuilding the release branch from a different subset of development branches.

Steps 1.8 and 1.9 are that path, and they stay documented for when a release
first needs them.
...
| job | runs on | does |
| --- | --- | --- |
| **`check`** | every push and pull request | `cargo fmt --check`, `clippy -D warnings`, `cargo test --workspace`, the wasm build, the commit-stamp check, and the shell suites |
| **`e2e`** | pull requests and `main`, after `check` | builds the preview stack in Docker and runs the Playwright suite against it |
...
#### 2.5.1 The advisory check, and why it denies warnings

`.github/workflows/advisories.yml` runs `cargo audit` nightly and on demand. It
is a **check**, not a gate: it blocks nothing, and what it produces is an issue
for a person to read.

**It refuses on reachability, not on severity.** `cargo audit` reads
`Cargo.lock`, which lists every crate any feature combination *could* pull in —
including optional dependencies of dependencies that nothing enables. So most
findings are against crates in no build graph at all, and severity does not
predict which. Measured on 2026-09-06 it was exactly backwards: both
**vulnerabilities** were unreachable (`rsa` behind an unenabled `sqlx-mysql`,
`webbrowser` behind the desktop feature), and both findings inside the server
binary were **warnings** (`event-listener`, and a yanked `spin`).

**So the run is `--deny warnings`**, and `.cargo/audit.toml` holds the ignore
list. Its single criterion is that the crate cannot reach us, and **every entry
carries the reason and how it was established** — an entry without one is
indistinguishable from something nobody looked at.

**The report becomes work by becoming an issue.** A failing run opens one
untriaged issue, or updates the open one if there is already one. Not one issue
per advisory: after the ignore list the steady state is no findings, so
per-advisory granularity buys nothing and a bad week would arrive as a burst.
Not the Actions tab alone either — a signal in a place nobody routinely opens is
not a signal, which is what #310 was about.

**`.github/dependabot.yml` watches the pinned actions**, weekly, and deliberately
not the crates: Dependabot reports newness and this check reports advisories,
and mixing them would bury the second in the first.
...
`/version.txt` in 2.7.4, and the defect #103 fixed.
...
**#288 R1: *is this change in the build?* is answered from the artefact, not
