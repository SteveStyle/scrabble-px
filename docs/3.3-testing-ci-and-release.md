...
The whole pipeline is here: the six-command runbook, what each environment is
for, and how a change is classified and merged. Start with
[Shipping a change](#shipping-a-change-the-full-sequence) if you have a
change ready to go; with [The seven types](#the-seven-types-of-change) if you are
about to raise an issue.
...
| --- | --- |
| **1. Subject stamp** | `app <X.Y.Z> api <M.N>: subject`. Read both out of the tree, not from memory:<br>`grep -m1 '^version' Cargo.toml`<br>`grep -m1 'API_VERSION: ApiVersion' crates/api/src/lib.rs` |
...
**1 — record the change.** One atomic change per commit, with the version stamp
...
**3 and 4 — look at it on preview, then use it.**
...
**5 — rehearse.** `./scripts/deploy-rehearsal.sh`. Same script, same images,
...
It refuses on nine counts; the full table is under
[The pipeline at a glance](#the-pipeline-at-a-glance). Needs `gh` authenticated.
**Run it bare** — piping it into `tail` reports the *pipe's* exit status, not
the script's.
...
**Why the dev restart matters.** `services.sh restart` kills whatever holds `:3000`/`:8080`, including an
...
**Why step 3 is on preview rather than in dev.** This is the functional pass, and it is the one thing no script can do.
...
**What rehearsal actually tests.** The same script, the same images and the same steps production gets, against
...
**It is also where a change with nothing to look at gets tested.** Step 3 asks
...
**The nine gates, and the one that is easy to overlook.** It refuses on nine counts. The commit must be a **valid ref**, **on the
...
**It reads the stack, not a browser.** The commit comes from preview's own
...
**Run it bare, and why that is not fussiness.** Piping into `tail` or `head`
...
**Why the re-stage has no exceptions.** This is the [B/C stage
...
**And it is enforced, not trusted.** The rehearsal gate compares `/health`
