...
Each section is *what to do* followed by *notes* carrying the reasoning. If you
are executing the process rather than studying it, these are the parts you need
— the notes are there when something surprises you.

| you are about to | where | go to |
| --- | --- | --- |
| **ship a change** | dev → preview → rehearsal → production | [Shipping a change — what to do](#what-to-do) — the six commands, end to end |
| **raise an issue** | — | [The seven types of change](#the-seven-types-of-change), then [the lifecycle rules](#the-change-lifecycle-branch-look-merge-release) |
| **commit** | dev | [Version numbers](#version-numbers-which-to-bump-and-when) — three checks, answered in a table |
| **cut or finish a release branch** | the repository | [Keeping a version branch current](#keeping-a-version-branch-current), then [merge first, and fast-forward](#releasing-a-version-branch-merge-first-and-fast-forward), then [delete the branch](#delete-the-branch-once-its-change-is-live) |
| **put production back to an earlier release** | production — rehearsal supports it so the rollback itself can be rehearsed | [Rolling back](#rolling-back) |
| **cope with production being broken** | production | [Emergency changes](#emergency-changes) — two commands, in order |
| **write a test plan** | — | [the template](templates/test-plan.md), and [how a test plan is built](#how-a-test-plan-is-built) for why |
| **check everything is in place** | all four | `./scripts/verify.sh`, or `--quick` for three seconds |

Two sections are reasoning only, and say so at the top:
[Why the process is shaped this way](#why-the-process-is-shaped-this-way) and
[How `deploy.sh` ships an image](#how-deploysh-ships-an-image). Neither is
needed to do anything.

Part of the lifecycle series — see [docs/README.md](README.md) for the full
sequence. Follows [Development](3.2-development.md). Once a version is live,
[Production Environment & Operations](3.4-production-environment.md) covers
the running system this pipeline ships onto (container topology, secrets,
inspecting and maintaining it) — the steady state, not the release
procedure.
...
- [Deleting a user](changes/41-user-deletion-test-plan.md) (#41) — functional,
  user-testable, and the plan that established the method.
- [Rate limiting](changes/25-rate-limiting-test-plan.md) (#25) — non-functional,
...
aged — which is how twelve merge-lane issues came to be left open with their
work shipped, each commit saying `Refs #N` where the merge lane takes `Closes`.
...
**What goes in *what*, and what goes in *notes*.** Two questions, and the split
falls where their answers differ:

- **What will a reader need every time they execute the process?** That is the
  *what* — the command, the rule, the sequence, the thing that must be to hand
  while doing it.
- **What will a reader need when they want to understand the process and the
  reasoning behind it?** That is the *notes* — why the rule is that way, the
  case that produced it, the alternative that was rejected.
...
justification below does not lose it; it stops it being in the way.
...
what `main` is for — the isolated lane, and the only one that releases from
...
[Releases are branches](#releases-are-branches) below.
...
### Releases are branches

Three lanes, named for the version slot each one releases into, so a lane and a
...
#### A lane is a milestone, and so is a release
...
| `fasttrack`, `minor`, `major` | queued — the lane is decided, the release is not |
| `0.4.26`, `0.5.0`, `1.0.0` | scheduled — this is the release it goes in |

**What puts a change in the merge lane is the artifact, not the audience.**
...
**`merge` is a fourth lane, and the odd one out.** Documentation and
...
That also explains why they accumulate. Every other lane has a release that
...
**Scheduling is moving an issue from its lane to a version.** Nothing is lost
in the move, because the version implies the lane: a patch came from
`fasttrack`, a minor from `minor`, a major from `major`. One slot per lane is
...
always go slower than the lane, never faster.** Wanting to go faster means the
lane was decided wrong, which is a decision to revisit rather than an exception
to make.

**A release takes the version of the highest lane in it.** Four fasttrack
...
**Fasttrack also asks that the change can be done *well* quickly.** Not merely
that it is small: a change worth thinking carefully about belongs in a slower
lane even when it is a few lines, because the lane sets how much deliberation
...
**And how many changes there are decides the lane too, not only what kind they
are.** A fasttrack release is one or two changes; that is its shape. Ten
...
This matters because the highest-lane rule can only ever be triggered by a
...
A lane needs deciding when the issue is raised, which is when the person
raising it knows most about the risk. A release cannot be decided then, because
...
same name — so a lane milestone is simply invisible to them.
...
**Within a lane, changes are sequenced.** Two grouped releases do not run at
once; the second starts when the first has shipped. It is across lanes that
...
fasttrack changes there and you have made a small grouped release that happens
to live on `main` — and an emergency arriving now cannot go out without taking
them along. The lane is only as fast as the shortest thing between `main` and
production.
...
- *How risky, and how long?* decides the **lane**.
- *Does anything behave differently?* decides the **version** — see
  [the seven types](#the-seven-types-of-change), whose table now states the
  version each implies.
- *Does the wire contract change?* decides the **api version**, which moves
  independently of both.

#### Keeping a version branch current

The rule that keeps the lanes independent, and the one that will be skipped if
it is not written down.

**Merges flow one way: from the faster lane into the slower ones, after every
...
structural branch. Never the other way: a slower lane's work reaching `main`
early is exactly what the lanes exist to prevent.
...
against final content. It also matters that merge-lane changes land on `main`
...
#### Releasing a version branch: merge first, and fast-forward
...
moved. Merge-lane work continues on `main` throughout a release, so `main`
moving is the normal case, not the exception — 0.6.0 collected eleven such
commits during its testing.

#### Delete the branch once its change is live
...
```bash
git push origin --delete issue-79-last-move-highlight
...
The remote half of this step may already be automatic (GitHub's *Automatically
delete head branches*), but the local half cannot be: nothing server-side
reaches your machine, and `git fetch --prune` removes the remote-tracking ref
while leaving the branch. That is exactly how `issue-33` and `issue-50`
outlived their own releases.

#### Notes on releases and branches

##### What closing a release's scope does not freeze
...
##### Two version numbers, counting different things
...
| --- | --- | --- | --- |
| **app** | `major.minor.patch` | which lane the release came out of | risk and duration — the table above |
| **api** | `major.minor` | the contract with the client | whether the wire broke, or only grew |

**The app version numbers the lanes, and a release takes the highest lane it
...
[Decide whether the API version moves](#decide-whether-the-api-version-moves).
...
##### The next structural release is 1.0.0

The version has been `0.4.x`, so the leading zero was inert and two slots were
carrying three lanes — grouped and structural both landing on the minor. The
...
does not say there will not be any. Under the lane mapping that is exactly what
...
##### Tooling is released by merging, so what tests it?
...
does](#evidence-follows-what-the-change-does-not-its-type) puts everything else.
...
##### Why `deploy.sh` needs no approval step of its own
...
##### Pull requests, and what they are for here
...
That is the release lanes. A merge-lane change puts `Closes #N` in the commit
instead, whether or not it uses a branch — see [Tracking a change to
production](#tracking-a-change-to-production).
...
**The rules, in one place.** Each has a subsection below carrying the reasoning
and the case that produced it; this is so you can find the one you need without
reading the rest.

| | rule |
| --- | --- |
| [merging](#when-to-merge-and-what-to-hold-back) | Testing does not require merging. Merge once, when deciding to release. |
| [branching](#when-to-branch) | Branch when something **else** is ready to ship, or when a change looks likely to need more than one round of feedback. |
| [living documents](#living-documents-and-the-standing-issue-that-carries-them) | A document that is never finished lives on `main` with a **standing issue**, which closes when its *subject* concludes rather than when an edit lands. |
| [where a document lives](#where-a-document-lives-about-the-work-or-a-product-of-it) | *About* the work → `main`, for its whole life. A *product* of the work → the branch, landing with it. Testing documents belong to the release, always; design documents to the issue. |
| [tracking](#tracking-a-change-to-production) | `Refs #N` in the commit — **except in the merge lane, where `Closes #N` is correct**, because nothing else will ever close it. |
| [types](#the-seven-types-of-change) | Every issue carries exactly one type label. |
| [evidence](#evidence-follows-what-the-change-does-not-its-type) | Evidence follows what the change **does**, not the label it carries. |
| [closing](#closing-an-issue-use-the-reason-not-a-label) | Use `stateReason`, not a label. |
| [documentation](#documentation-is-owed-by-every-type-not-just-its-own) | Every type owes documentation; the `documentation` label means the *whole* change is documentation. |
| [splitting](#one-change-one-type--split-the-issue-otherwise) | One change, one type. An issue touching both tooling and the app is two issues. |
...
#### Living documents, and the standing issue that carries them
...
document is finished whenever it correctly describes the state it reports on,
which is exactly why it belongs on `main` — a testing report kept on a branch
is a report nobody can read while the testing it tracks is happening.
...
#### Where a document lives: about the work, or a product of it
...
- **Testing documents belong to the release. Always.** A test report, a testing
  ...
  found in neither.
- **Technical design documents belong to the issue** that produced them. A
  design note is about one change, outlives no release, and is read by whoever
  next touches that code.
- **One document has exactly one owner.** Where a document appears to belong to
  ...
  issues collaborating on one product get a **master issue** that owns the
  product and its documents, and the others become its parts.
...
**Except in the merge lane, where `Closes #N` is correct.** A documentation
...
**A branch is optional here, and that is the point.** What this lane rules
...
- **Risk.** A branch buys CI's verdict before the change is live, which
  counts for more in this lane than elsewhere: a merge-lane change has no
...
**The lane records when a change goes live, not how big it is.** Tooling is
merge lane and can carry a lot of functionality and testing — rebuilding the
release gate around named CI runs was merge lane, and so was the `status.sh`
rework. A substantial new document is the same shape. Neither belongs in a
slower lane, because neither reaches users through a release: there is
...
Everything else the release needs — waiting for CI, snapshotting the database,
applying migrations as their own step, smoke-testing the result, tagging what
shipped, moving the dev version on — happens inside those. You do not type them
and cannot forget them.

**1 — record the change.** [1](#notes) [2](#notes) One atomic change per commit, with the version stamp
first (see [Version numbers](#version-numbers-which-to-bump-and-when)). Build,
run and unit-test it before committing; that is part of making a change, not a
stage after it.
...
**2 — push.** `git push origin main` starts CI. You need not watch it;
`deploy.sh` waits for the verdict and refuses a commit that has not passed. To
ask early: `./scripts/ci-status.sh --run push:<branch> --wait`.

**3 and 4 — look at it on preview, then use it.** [3](#notes)
`./scripts/deploy-preview.sh` builds a fresh checkout of `HEAD` and waits until
it answers. Then open `http://localhost:8081` and *use the change*. If something
is wrong, fix it and start again from step 1 — the new commit invalidates the
rehearsal host, and the deploy will refuse until you have redeployed it.

**5 — rehearse.** [4](#notes) [5](#notes) `./scripts/deploy-rehearsal.sh`. Same script, same images,
same steps production gets, against a machine built to match it. Nothing here is
a release: no `prod-*` tag, no version bump, no milestone closed, and the output
says so on every run.

Where a change's correctness depends on conditions preview cannot reproduce,
write a script and run it here:
...
It refuses on nine counts [6](#notes) [7](#notes); the full table is under
[The pipeline at a glance](#the-pipeline-at-a-glance). Needs `gh` authenticated.
**Run it bare** [8](#notes) — piping it into `tail` reports the *pipe's* exit status, not
the script's.

Then, without being asked: snapshot the database, apply migrations as a separate
step so a failure there is not an outage, swap the containers, smoke-test the
public URL for the exact version, tag the deployed commit `prod-<version>`, and
move the dev version one patch ahead. If the migrations or the smoke test fail
it calls `scripts/rollback.sh` itself.
...
**[1]** Three rules, in this order, because anyone can forget a step:
...
**[2] Why the dev restart matters.** `services.sh restart` kills whatever holds `:3000`/`:8080`, including an
...
**[3] Why step 3 is on preview rather than in dev.** This is the functional pass, and it is the one thing no script can do.
...
**[4] What rehearsal actually tests.** The same script, the same images and the same steps production gets, against
...
**[5] It is also where a change with nothing to look at gets tested.** Step 3 asks
...
**[6] The nine gates, and the one that is easy to overlook.** It refuses on nine counts. The commit must be a **valid ref**, **on the
...
host must be running it**. The full table is under
[The pipeline at a glance](#the-pipeline-at-a-glance). Needs `gh` authenticated.
...
**[7] It reads the stack, not a browser.** The commit comes from preview's own
...
**[8] Run it bare, and why that is not fussiness.** Piping into `tail` or `head`
...
milestone in your head. It prints version milestones oldest first, then the lanes
...
[1](#notes-on-rolling-back) [2](#notes-on-rolling-back)
...
investigation. Read [Rolling back](#rolling-back) before you need it, not
during.
...
#### What it still does, and what it deliberately does not
...
#### The retrospective is the price
...
The issue arrives with no type and no lane, deliberately: what it turns into is
...
sequenceDiagram
    ...
    GH-->>Dev: 1.d CI runs · 1.e ci-status.sh --run push:<branch> --wait → fmt · clippy · test · wasm · stamp
...
./scripts/ci-status.sh --run push:my-branch --wait     # what you ask at 1.e
...
merge lane routinely does not. Absence is therefore a pass, which is the shape
