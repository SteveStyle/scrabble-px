...
| gate | run | mandatory |
| --- | --- | --- |
| merging a branch | its `pull_request` run, where there is one | advisory — not every change has a PR |
| production deploy | the push to `main`, with `e2e` | yes |
