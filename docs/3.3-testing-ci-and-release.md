...
| **1.5** | rehearse the release | the only test of the release *mechanism* |
...
**A release branch** — normal, and only for the lanes that group changes:
...
| [where a document lives](#294-where-a-document-lives-about-the-work-or-a-product-of-it) | *About* the work → `main`, for its whole life. A *product* of the work → the branch, landing with it. Testing documents belong to the release, always; design documents to the issue, or to the workstream's parent issue. |
...
| **test plan** | the **project**, or the issue where there is none | the project's folder — `docs/changes/releases/<version>/` — else the issue folder. It **stays**: it is the documentation for the regression tests it produced | never; it outlives the change |
| **test report, testing status** | the **project** | `docs/changes/releases/<version>/`, named for the project's **main release** | stays |
...
**The folder is named for a version, and owned by a project.** A project is
known by its main release — `1.0.0` — and its testing documents live under that
name whether the work ships in one release or several. Where part of a project
ships early as a patch, that patch does not earn a folder of its own: its record
is the release log entry, and its testing belongs to the project that produced
it. This keeps the rule *"testing documents are never filed by judgement"* while
admitting that a project and a release are not the same thing.
