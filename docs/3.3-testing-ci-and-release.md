...
On success it also **bumps the working tree to the next patch, commits and pushes**, so production occupies the version just shipped and the tree stays permanently one ahead. That used to be a separate step you had to remember, and it is only ever correct immediately after a successful deploy, which is exactly where it now happens. `DEPLOY_SKIP_BUMP=1` opts out.
