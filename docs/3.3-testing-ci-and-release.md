...
![The release flow: Dev, Staging and Production environments with local git
and GitHub to the side, and the nine runbook steps as arrows between
...
Dotted arrows carry no code — they are questions rather than movement, and
two of them are what `deploy.sh` refuses on.
