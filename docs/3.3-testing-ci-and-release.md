...
> **The image below is out of date.** `release-flow.mmd` has been updated for
> the four environments — rehearsal added, the gate moved onto it — but the
> `.svg` has not been re-rendered, so it still shows three. Follow the numbered
> runbook above rather than the picture until #35 closes.
