...
A release is a scope, and a scope changes while you are working on it. So the
unit is a branch named for the version it will ship: `0.5.0`.

**`main` is what is live.** Merging into `main` is the act of committing to a
release, not a step on the way to one — its history is the release history, and
rolling back is walking back along it.

```text
issue-79-last-move-highlight ─┐
issue-71-one-game-model      ─┼─▶  0.5.0  ─────────────▶ main ──▶ production
issue-25-rate-limiting       ─┘                 ▲
                                                │
issue-83-urgent-fix ────────────▶ main ──▶ production   (forward-merged)
```

**A feature branch merges into its version branch, not into `main`.** The
version branch is where several changes are integrated and seen together —
which is the whole gain, and the reason this is worth more than "don't merge to
main early". A release of one change barely needs it; a release like #71, a
block of issues that only makes sense together, needs somewhere to become one
thing before it becomes live.

**`Cargo.toml` on the branch says that version from its first commit**, so
every commit's stamp is true of its own tree and `check-commit-stamp.sh` is
satisfied without anybody thinking about it.

**Pick the number when you create the branch.** Any `major-function` or
`minor-function` in the scope makes it a minor; only `bug`, `documentation`
and tooling makes it a patch. That is a judgement about work you can see, made
once, rather than a default applied after the fact.

#### Two version lines at once

The point of the model. Functional work can sit on `0.5.0` for weeks while a
bug fix goes out from `main` as `0.4.26`.

**Then merge `main` into every open version branch, immediately.** Not a tidy-up
— skip it and the eventual `0.5.0` release quietly reverts a fix that is
already live, and it will be found by the person who reported the bug in the
first place.

```bash
git checkout 0.5.0
git merge main          # bring the released fix forward
git push
```

The version stamps do not fight: `check-commit-stamp.sh` reads each commit's
version from *its own* tree, so `0.4.26` commits sitting in a `0.5.0` branch
are all correct about themselves. Merge commits are skipped entirely.
