...
| route | steps |
| --- | --- |
| **Repository Change** — documents, scripts, workflows | 1.1, 1.2, then push. Live on `main`, so no 1.3–1.7 |
| **Production Release** — anything in the image | 1.1 – 1.7 |
...
branch.** It is not needed when a release comes from a single branch, which is
how every release so far has shipped — `0.7.1` and the three before it are
single-parent commits and no release branch was made.

**What it buys is a changeable scope.** The release branch is made by merging the
development branches that are going in it, so dropping one means **rebuilding the
branch from a different subset** rather than reverting work out of it. That is
why nothing is ever committed to a release branch directly: a change made there
exists nowhere else and would not survive the rebuild.

Steps 1.8 and 1.9 are that path, and they stay documented for when a release
first needs them.

**Emergency is not a route.** It is a process for when production is broken now:
1.11 first, and 1.13 only if going back cannot help.

**And it saves less than it used to.** It skips the preview and rehearsal gates,
which before #214 meant skipping two image builds. Now the artefact is built once
per commit and both environments reuse it, so what is skipped is two transfers
and two smoke tests — the build happens either way, because an emergency is a new
commit. What is left to save is the CI wait and the human steps. Rolling back
remains far faster than either: it is a retag and a restore, with no build and no
transfer at all.
