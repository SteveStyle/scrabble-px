...
above dissolves. On a branch the PR body carries the keyword; committed
straight to `main` — which a doc typo may be — the commit has to, because
there is no branch, so no merge, and no PR body to put it in. Without it
nothing ever closes the issue: `deploy.sh` closes issues in the milestone of
