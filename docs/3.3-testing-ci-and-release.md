...
```bash
./scripts/deploy-staging.sh at prod-0.4.12   # rehearse it, as for any deploy
...
What this does **not** roll back is the database. `sqlx::migrate!` only
applies pending "up" migrations; an older image simply won't know about
migrations added since, and a schema change that the old code cannot read
is not recoverable this way. See the volume-level options at the end of
