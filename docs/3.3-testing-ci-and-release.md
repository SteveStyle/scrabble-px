...
| released as | a patch | a minor | a major |
| released with | nothing — it goes on its own | whatever else is in the group | the changes it affects, and nothing else |
...
**A lane is the fastest route a change may take, not the one it must.** A
fasttrack change can be scheduled into a minor release that is going out soon
rather than earning a patch release of its own — riding along costs nothing and
saves a release. The asymmetry is the rule: **scheduling can always go slower
than the lane, never faster.** Going faster means the lane was decided wrong,
and that is a decision to revisit rather than an exception to make.
...
**The app version numbers the lanes, one slot each.** A structural release
moves the major, a grouped one the minor, an isolated one the patch. That is
the whole rule; there is nothing else to weigh.
