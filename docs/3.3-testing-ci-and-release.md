...
Contents rather than a version number, so it changes when the *bundle* changes:
...
- **but every release changes it, including server-only ones.** The Dockerfile
  compiles `TILE_LITE_ELITE_BUILD_ID` — the commit — into the wasm so the
  client can show `app 0.5.0+3d821e9` in the top bar, and the hash is taken
  over the built files, so the commit is inside the thing being hashed. Change
  the commit and the hash moves, whatever the change was.

**So a reload on a server-only release is working as built, and the claim that
it would not was wrong** rather than the behaviour. It is honest and harmless:
the client reloads and gets the same client back, having spent a page load.

Left alone deliberately. The alternatives — a build id in a side file the
client fetches separately, or a version read from `/health` at runtime — both
add a mechanism to maintain, and the second misreports itself during exactly
the skew the reload exists to detect. Nobody has complained about the reload;
the observation came from noticing one, not from being disrupted by it.
...
rehearsal host is already running `X`, and refuses unless `scripts/` is clean
and on `main`. If `X` is the change to `deploy.sh` itself, then `X` is main's
...
merging into it is the act of releasing — and `deploy.sh` enforces that
directly: a production deploy refuses unless the tooling it is running from is
on `main` and clean. Trying to deploy from the version branch is refused, which
is the right answer to the wrong sequence.
...
It refuses on five counts: the commit must be **on the remote**, **CI must
have passed** for it, production's database must not have **moved past the
schema** this build knows, and **both preview and the rehearsal host must be
running it**. Needs `gh` authenticated.
