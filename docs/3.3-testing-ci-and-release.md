...
It is also not merely ceremonial when moving backwards — see the migration
note at the end of this section for what `at` does that a plain re-run
would not.
...
**What this does not roll back is the database — and going backwards across
a migration is a hard stop, not a degraded mode.** `sqlx::migrate!` only
applies pending "up" migrations, and on startup `Migrator::run` validates
that every migration *recorded in the database* is one the binary knows
about. It isn't lenient by default (`ignore_missing: false`), so an older
image meeting a database that carries a newer migration fails with
`VersionMissing(<version>)` and **the server does not boot**. Nothing is
corrupted; it simply refuses.

This is why re-staging a rollback uses `at <ref>`, which wipes the staging
volume before building. Staging's database still holds Brian's migration;
Andy's binary would refuse to start against it. The wipe is what makes the
rehearsal possible at all, not a convenience.

Production has no such wipe, which sets the real boundary: **a rollback is
safe as long as the target is not behind a migration that production has
already applied.** In the Andy/Brian case that holds, because Brian never
reached production — its migration only ever ran in staging. Rolling back
*across* a migration that did ship is a restore-from-backup exercise, not a
redeploy; see the volume-level options at the end of
[Staging Environment](#staging-environment) and
[3.4](3.4-production-environment.md#backups).
...
**The transfer**: build both images from the worktree, `docker save` and gzip them, `scp` them plus *that checkout's* `docker-compose.yml` to the VM, `docker load` there, `docker compose up -d`, prune. A few minutes, almost all of it the local build. Configurable via env vars (`DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`, `DEPLOY_REMOTE_DIR`) if the target ever changes — see the script header.
