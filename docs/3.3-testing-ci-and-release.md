...
**It is really a client version, not an API version.** The name is
historical: it was introduced on the assumption that a client update implied
a change in API behaviour, and that turned out not to be true — a client can
change without the wire contract moving at all, which is exactly what 2.9
is. What the number actually does, and all it does, is tell a client what to
do about updates. So asking "was this a DTO change / a bug fix / a
refactor?" is asking the wrong question:

| | what it tells a client | desktop | web |
| --- | --- | --- | --- |
| **major** | There is a new version and you cannot continue without it. | Mandatory update | Automatic reload |
| **minor** | There is a new version; you still work without it. | Optional update | Automatic reload |
| none | There is nothing new for you. | — | — |

The web client reloads on either, because a reload is free there. Desktop
keeps the distinction, because the remedy is a download and install.

So the question is only: **is there a new client build, and can the old one
carry on?** Everything else follows.

- A change anywhere in what the client compiles — `crates/ui`, or a shared
  crate like `rules-shared` — produces a new client version, so it gets a
  bump. **Including a pure bug fix**: skew is the only thing that makes an
  open tab reload, so a client fix shipped without a bump reaches nobody who
  already has the app open. Sharply true for 2.9, where the bug *is* a tab
  getting stuck across a deploy — without the bump the fix could not have
  reached the tabs it was written for.
- A server-only change announces nothing, because there is no new build to
  fetch. 0.4.16 changed how the server broadcasts game state and correctly
  moved nothing: every client already handled those payloads.

The case that has actually been got wrong was read as server-only and
wasn't: 0.4.13 added the `spicy` edition to the registry in `rules-shared`,
which the client compiles too — so there *was* a new client build, carrying
an edition the old one had never heard of. It shipped at 2.5 unchanged, so
no tab ever reloaded and none could offer the edition that had just shipped.
...
The name is worth reading past. `API_VERSION`, `HealthDto.api_version` and
the `api M.N` in every commit subject all describe a *client* version, and
renaming them is a real change rather than a cosmetic one — the field is on
the wire, so it would need adding alongside the old one and removing later.
Untangled deliberately or not at all; in the meantime, read "api" here as
"client".

**`check-commit-stamp.sh` will not catch this for you.** Its warning watches
`crates/api/src/` and the edition registry, which is not where the trigger
lives — "is there a new client build" is a question about the whole
dependency tree of `crates/ui`. Treat the warning as a net with holes in it,
not a definition.
...
./scripts/deploy.sh <ref>      # or a specific commit — see Rolling back
```

It checks that the commit is **on `origin/main`**, that **CI passed for
it**, that **staging is running it**, and that **production's database
hasn't already moved past the schema this build knows** (which would leave
the container unable to boot — see [Rolling back across a
migration](#rolling-back-across-a-migration)). Needs `gh` authenticated
(`gh auth login`). `DEPLOY_SKIP_CI=1` bypasses the CI gate, for a genuine
emergency where GitHub itself is the problem — not as a way around a
failing test.
