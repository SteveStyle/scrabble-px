...
| **deliveries** | what ships, in what order, by which route |
...
**A branch per project**, named for its issue. Not a judgement call each time —
...
| **test plan** | the **project**, or the issue where there is none | the project's folder — `docs/changes/projects/<name>/` — else the issue folder. It **stays**: it is the documentation for the regression tests it produced | never; it outlives the change |
