...
| --- | --- |
| **ship a change** | [Shipping a change — what to do](#what-to-do) — the six commands, end to end |
...
| **commit** | [Version numbers](#version-numbers-which-to-bump-and-when) — three checks, answered in a table |
| **cut or finish a release branch** | [Keeping a version branch current](#keeping-a-version-branch-current), then [merge first, and fast-forward](#releasing-a-version-branch-merge-first-and-fast-forward), then [delete the branch](#delete-the-branch-once-its-change-is-live) |
| **go back to what was working** | [Rolling back](#rolling-back) |
| **cope with production being broken** | [Emergency changes](#emergency-changes) — two commands, in order |
| **write a test plan** | [How a test plan is built](#how-a-test-plan-is-built) |
| **check everything is in place** | `./scripts/verify.sh`, or `--quick` for three seconds |
