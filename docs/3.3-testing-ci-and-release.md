...
**So the default is to batch.** A fasttrack change normally waits for the next
minor release, because one release costs the same attention as another and
carrying four changes through it costs a quarter as much per change. A patch
release of its own is for the two cases that earn it: something urgent enough
that waiting is worse, or something you simply want live now. `0.4.24` was the
second kind — one appearance fix, released alone because it was wanted.
