...
**Blue boxes run code; sand boxes store it.** Dotted arrows carry no code
either way — they are questions rather than movement, and two of them are
what `deploy.sh` refuses on.

What the picture is for:

- **Only step 8 reaches production, and only from your machine.** GitHub
...
- **Staging is on your machine, not beside production.** The only lines
  between them carry production's version number into staging (step 5) and
  compare the two afterwards (step 9). So a staging failure cannot take
  production with it, and staging judges a release independently rather
  than sharing its fate.
- **Steps 1–7 are a loop**, run as often as the change needs. Only the pass
  that reaches step 8 is unusual, and the two gates are why: an edit after
