...
#### Three lanes, two numbers

Worth naming, because it is unresolved. The project is pre-1.0, so the version
is `0.MINOR.PATCH` and there are only two slots to spend on three lanes:
grouped and structural both land on the minor.

Today that is survivable, because the distinction they lose is carried
elsewhere — the api version says whether the wire broke, and the process says a
structural change ships alone with a design note. The lane is visible in how
the change was made, not in its number.

The alternative is 1.0.0, which would give all three lanes a slot of their own.
That is a statement about the project rather than about a release: pre-1.0
means "the shape may still change", which is exactly what the one-game-model
work is doing. Worth deciding when that ships, not before.
