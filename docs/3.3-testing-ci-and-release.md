...
```bash
./scripts/restore-prod-snapshot.sh                    # list what's there
./scripts/restore-prod-snapshot.sh pre-0.4.13-...tgz  # restore (asks you to
                                                      # retype the name)
./scripts/deploy-staging.sh at <ref>                  # then the usual two
./scripts/deploy.sh <ref>                             # steps
```
