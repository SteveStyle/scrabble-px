...
chunk lands, so it cannot belong to any one chunk's branch; and the parent
...
- **Technical design documents belong to the issue** that produced them — which
  ...
  note outlives several by design, because its chunks are separate branches and
  a note on one of them is invisible to the others.
...
| **test report, testing status** | the **release** | `docs/changes/releases/<version>/` | stays with the release |
