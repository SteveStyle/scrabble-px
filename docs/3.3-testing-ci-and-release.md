...
That is the release lanes, which is where pull requests are. A merge-lane
change has no branch and no PR, and its commit does say `Closes #N` — see
[Tracking a change to production](#tracking-a-change-to-production).
...
These go **straight to `main`**. A branch and a pull request are the ceremony
this lane exists to avoid: the change is live at merge, and it is not
something you would user-test. So the commit carries the keyword itself —
there is no PR body to put it in — and without it nothing ever closes the
issue, because `deploy.sh` closes issues in the milestone of a *deployed
version* and `merge` is never one.
