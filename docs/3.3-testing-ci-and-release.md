...
Six activities and one loop. No script names, ports or job names here on
purpose — those are in the numbered steps below. This is the shape of the
thing.
...
flowchart TB
    ...
    DEV["try it locally"]
    STAGE["rehearse on staging<br/>user test here"]
    SHIP["release"]
    PROD(["production"])
    ...
    PUSH --> CHECK
    PUSH --> DEV
    DEV -.->|"more changes"| WRITE
    DEV --> STAGE
    STAGE -.->|"more changes — staging is stale again"| WRITE
    STAGE --> SHIP
    CHECK -. "must have passed for this exact change" .-> SHIP
    STAGE -. "must be the change that was staged" .-> SHIP
    SHIP --> PROD
```

The two dotted arrows into **release** are why the loop can't be
short-circuited. Releasing refuses unless the checks passed *for this
change* and staging is running *this change* — so any edit made after
staging puts you back at the top, whether or not you intended to go there.

- **The loop is meant to run many times.** Going round between *make a
  change* and *try it locally* costs nothing. Only the last pass, the one
  that reaches *release*, needs staging to be current.
- **Nothing reaches production except the release step**, and it is the
  only thing in the picture that can. The checks run somewhere with no
  access to production at all, and staging deliberately runs on your own
  machine rather than beside production — so that a staging failure cannot
  take production down, and so staging judges a release independently
  instead of sharing its fate.
- **The release records itself.** Tagging what shipped and moving the
  version on afterwards are not steps you perform.
