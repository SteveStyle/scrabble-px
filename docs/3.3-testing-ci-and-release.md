...
**A branch is optional here, and that is the point.** What one buys is CI's
verdict before the change is live, which counts for more in this lane than
elsewhere: a merge-lane change has no preview or rehearsal stage behind it,
so landing it is the whole release. A doc typo does not need that; a tooling
change that could break a deploy might. What this lane rules out is the
*obligation* — a branch and a pull request for every typo.
