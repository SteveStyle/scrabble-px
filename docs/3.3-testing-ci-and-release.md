...
production by one (see step 11).

#### Decide whether the app version moves

Usually it doesn't. The app version is bumped **once per production
release**, at step 11, immediately after deploying — not per commit and not
at the start of a change. That is what keeps the working tree permanently
one ahead of production, so no commit is ever built carrying a version
number that is already live.

Patch for fixes, minor for new functionality. It is a single line in the
root `Cargo.toml`, since the version is workspace-inherited.
...
**11. Bump the dev tree to the next patch** — production now occupies the version you just shipped, so the working tree moves ahead by one and stays there until the next deploy. Do this immediately, not at the start of the next change: it's what keeps the working tree permanently leading production by one version, so no commit is ever built carrying a version number that's already live. Patch for fixes, minor for new functionality.

The version is workspace-inherited, so this is a single line in the root `Cargo.toml`:

```toml
[workspace.package]
version = "0.4.11"   # was 0.4.10 — the version now in production
```

Then commit and push it on its own:

```bash
cargo check                                   # refresh Cargo.lock's workspace versions
git commit -am "app 0.4.11: bump dev version following production release"
git push origin main
```

See [4.1 Configuration — Versioning](4.1-configuration.md#versioning) for why the two version numbers move independently; the API contract version is *not* bumped here, only on a real wire change.
...
sequenceDiagram
    ...
    Dev->>GH: 11. bump [workspace.package] version — dev leads prod by one again
```
