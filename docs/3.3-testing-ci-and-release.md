...
`crates/api/src/` and the edition registry, because that is where an
observable change usually lands — but "observable" is about behaviour, not
location. Making aborted games removable (2.8) changed only
`game_state.rs`, so nothing warned; the rule still applied. Treat the
warning as a net with holes in it, not a definition.
