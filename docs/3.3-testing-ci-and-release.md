...
| **preview** | the real images, in Docker, on the laptop | *looking at it* — user testing, phone screenshots | `./scripts/deploy-staging.sh` |
...
**A naming leftover**: `deploy-staging.sh`, `docker-compose.staging.yml`,
`Caddyfile.staging` and the `tile-lite-elite-staging-data` volume all still
say "staging". They are the **preview** environment. Renaming them touches
CI and the rendered diagrams, so it is tracked separately rather than
buried in the change that introduced the split.
...
flowchart LR
    ...
    subgraph look["Staging — look at anything, any time"]
        direction TB
        s1["deploy-staging.sh at &lt;branch&gt;"]
    end
...
**Staging is not a step towards production.** It is where you look at
something. `./scripts/deploy-staging.sh at <branch>` builds any ref with no
merge, so "show me this on my phone" and "rehearse the release" are separate
acts that happen to use the same environment. Most staging deploys are the
first kind.
...
requires staging to be running the *exact* commit being deployed. A merge
commit is a new commit that staging has never run, so it forces a rebuild
...
feedback from a phone, each a fresh commit, each invalidating staging and
...
| type | label | what it owes |
| --- | --- | --- |
| documentation | `documentation` | nothing beyond review — no deployment exists for `docs/**` |
...
| defect fix | `bug` | a regression test that fails before the fix and passes after |
| minor functional change | `minor-function` | user testing on the real artifact |
| major functional change | `major-function` | a design note in `docs/2.x` *first*, then user testing |
...
`deploy.sh`, `deploy-staging.sh` and `rollback.sh` are not in the image, and
...
**`Caddyfile` is the trap.** It reads like web-server configuration, but the
image copies `crates`, `Cargo.toml`, `Cargo.lock`, `.cargo`, `old-crates`
and `Caddyfile` — and nothing else. So `Caddyfile` changes the app;
`docs/` and `scripts/` cannot.
...
```bash
git add -A && git commit -m "app 0.4.18 api 2.9: ..."   # 1  record it
git push origin main                                     # 2  hand it to CI
./scripts/deploy-staging.sh                              # 3  rehearse it
#                                                        # 4  use it yourself
./scripts/deploy.sh                                      # 5  ship it
```
...
| user testing | staging, step 4 | does it do what was actually wanted |
...
#### Step 3 — rehearse on staging, then use it

`./scripts/deploy-staging.sh` builds a fresh checkout of `HEAD` and waits
...
happens on staging rather than in dev because staging is the last point
...
invalidates staging, and the deploy will refuse until you have rebuilt it.
...
schema** this build knows, and **staging must be running it**. Needs `gh`
authenticated.
...
| `./scripts/deploy-staging.sh at <ref>` | wipe staging and build a specific commit |
| `./scripts/deploy-staging.sh at prod` | seed staging from production's live commit — do this before testing a **migration**, so it runs against production's schema rather than an empty volume |
| `./scripts/deploy-staging.sh verify` | confirm staging and production match |
| `./scripts/deploy-staging.sh reset` | wipe staging's database |
