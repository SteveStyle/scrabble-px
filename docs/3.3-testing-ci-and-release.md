...
A merged branch that outlives its release is not harmless. `status.sh` reads
`issue-<N>-*` branch names to say what is being worked on, so a branch left
behind reports finished work as in progress — and the next person to look has
to open it to find out which.
