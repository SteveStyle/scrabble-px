...
what `main` is for. **A minor version is not bumped into — it is branched
to**, and that decision is made when the branch is created, with the scope in
...
Two ways to ship, and the first thing to settle is which one you are in.

| | **Small and independent** | **Grouped** |
| --- | --- | --- |
| what | a fix, a tweak, a doc change | several changes that only make sense together |
| where | branch → `main` | branch → version branch → `main` |
| pull request | no | yes, a draft from day one |
| released as | a patch | a minor or major |
| merged when | user testing passes | the whole scope is done |

**Small changes go to `main` as soon as they pass user testing, and ship.**
Exactly as before. `main` stays permanently releasable, and nothing about a
larger change in flight can hold up a fix — which is the whole reason for the
split.

**Grouped changes get a branch named for the version they will ship** —
`0.5.0` — with feature branches merging into it rather than into `main`. That
...
the one-game-model work does, being a block of issues that is meaningless one
at a time.

**The two questions are independent**, and conflating them is the mistake to
avoid:

- *Does this need grouping?* decides the **process** — straight to `main`, or a
  version branch.
- *Does this contain functional change?* decides the **version** — any
  `major-function` or `minor-function` makes it a minor, everything else a
  patch.

So a lone functional change can go straight to `main` and ship as `0.5.0`
without a version branch. It is grouping that earns the branch, not the version
number.

**User testing gates a merge, on both tracks.** A small change is tested in
preview before it goes to `main`, as now. On the grouped track the same gate
applies to each change going into the version branch — and then again to the
whole scope before it goes to `main`, because a group can be wrong in ways
none of its parts are. The second pass is the one that would be tempting to
skip and is the reason the branch exists.

**`main` is what is live.** Merging into it is the act of committing to a
release, not a step on the way to one — its history is the release history, and
rolling back walks back along it.

```text
issue-83-urgent-fix ──────────────▶ main ──▶ 0.4.26 live
                                     │
                                     │ forward-merge
                                     ▼
issue-71-one-game-model ─┐
issue-25-rate-limiting   ─┼──────▶ 0.5.0 ────▶ main ──▶ 0.5.0 live
issue-79-highlight       ─┘
```

**`Cargo.toml` on a version branch says that version from its first commit**,
so every commit's stamp is true of its own tree and `check-commit-stamp.sh` is
satisfied without anybody thinking about it.

#### Keeping a version branch current

While `0.5.0` is open, `main` goes on releasing patches. **Merge `main` into
every open version branch after each one, immediately.** Not a tidy-up — skip
it and the eventual `0.5.0` release quietly reverts a fix that is already live,
and the person who reported the bug is the one who finds out.
