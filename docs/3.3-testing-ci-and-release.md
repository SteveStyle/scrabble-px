...
**Blue boxes run code; sand boxes store it.** Note `git commit` is a loop
on *local git* rather than an arrow out of Dev: the working tree and the
history are both in that box, and nothing flows out of the running dev
environment.
...
**1.c Push, and get the answer.** Unlike the two before it, this step has a
*response*: the push starts CI, and CI either passes or fails.
...
answer too). It exits non-zero on anything but success, so it chains:
`git push origin main && ./scripts/ci-status.sh --wait`.
