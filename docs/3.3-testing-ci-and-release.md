...
Three patterns, chosen by risk and by how long the change will take. The first
thing to settle about any change is which one it is in.

| | **Isolated** | **Grouped** | **Structural** |
| --- | --- | --- | --- |
| what | one small independent change, or an urgent fix | several functional changes released together, or one larger one | changes the shape of the system: architecture, a significant refactor |
| risk | low, and contained | moderate — the parts interact | high, and hard to reason about a piece at a time |
| timescale | hours | days to weeks | weeks to months |
| lives on | `main` | a version branch, `0.5.0` | a version branch of its own |
| released as | a patch | a minor | a minor or major, and alone |
| owes | user testing | user testing per change, then again for the whole scope | a design note in `docs/2.x` **first**, then all of the above |
...
#### The major slot is not available yet

The project is `0.4.x`, so the leading zero is inert and there are two usable
slots for three lanes: grouped and structural both land on the minor.

Survivable today, because what they lose is carried elsewhere — the api version
says whether the wire broke, and the process says a structural change ships
alone, with a design note, having been seen on its own branch. The lane is
visible in how the change was made rather than in its number.

Fixing it means `1.0.0`, and that is a statement about the project rather than
about a release: pre-1.0 means "the shape may still change", which is exactly
what the one-game-model work is doing. **Worth deciding when that ships** — it
is the change that both earns the major slot and stops needing it.
