...
| **[The process, and why it is shaped this way](#2-the-process-and-why-it-is-shaped-this-way)** | understanding it | the environments, the lanes, the rules, and what each is for |
...
Numbers match the sections below. `deploy.sh`'s internal build-and-ship (build
locally → `scp` → `docker load` → `up -d` → `sqlx migrate`) is a single step
here, as is the version bump it performs itself; both are spelled out in [How
`deploy.sh` ships an image](#213-how-deploysh-ships-an-image).
...
Seven steps ship a change. The rest are numbered too, and are **not part of a
normal release** — they are there when something is wrong or when a release is
being managed rather than shipped.
...
| **1.3** | look at it on preview | the same images production will get, locally |
| **1.4** | use it yourself | the one judgement no script makes |
| **1.5** | rehearse | the only test of the release *mechanism* |
...
| **1.8** | roll back production | go back to a release that worked |
| **1.9** | roll back across a migration | database and images together, or the container will not boot |
| **1.10** | ship an emergency change | only when going back cannot help |
| **1.11** | keep a version branch current | forward-merge after every release |
| **1.12** | release a version branch | merge first, fast-forward, then ship |
| **1.13** | delete a branch once its change is live | merged means done, not deployed |
| **1.14** | diagnostics | ask a question without changing anything |

**Which steps a change needs depends on its lane**, and the lane comes from the
issue's type — see [A lane is a milestone, and so is a
release](#281-a-lane-is-a-milestone-and-so-is-a-release):
...
| **minor / major** — grouped into a version branch | 1.1, 1.2 on the branch; then 1.11, 1.12, and 1.3 – 1.7 for the release |
| **emergency** — production is broken now | 1.8 first; 1.10 only if that cannot help |
...
Use `Refs #N` in the body — **except in the merge lane, where `Closes #N` is
correct**, because nothing else will ever close it.

**Proved by:** `cargo test --workspace` exits 0, `check-commit-stamp.sh` exits
0, and `http://127.0.0.1:3000/health` reports the build you just made rather
...
Same script, same images and same steps production gets, against a machine
built to match it. Nothing here is a release: no `prod-*` tag, no version bump,
no milestone closed.
...
./scripts/deploy.sh <commit-ish>       # a specific commit — see 1.8
```
...
### 1.9 Roll back across a migration
...
### 1.11 Keep a version branch current

**Where:** development machine. **After every release, and once more when the
release's scope closes.** Merges flow one way — from the faster lane into the
slower ones:
...
If `--ff-only` fails, `main` has moved: forward-merge (1.11) and start the
...
### 1.13 Delete a branch once its change is live
...
#### 2.4.2 How a test plan is built
...
[Releases are branches](#28-releases-are-branches) below.
...
#### 2.8.1 A lane is a milestone, and so is a release
...
### 2.9 The change lifecycle: branch, look, merge, release
...
| | rule |
| --- | --- |
| [merging](#291-when-to-merge-and-what-to-hold-back) | Testing does not require merging. Merge once, when deciding to release. |
...
| [living documents](#293-living-documents-and-the-standing-issue-that-carries-them) | A document that is never finished lives on `main` with a **standing issue**, which closes when its *subject* concludes rather than when an edit lands. |
| [where a document lives](#294-where-a-document-lives-about-the-work-or-a-product-of-it) | *About* the work → `main`, for its whole life. A *product* of the work → the branch, landing with it. Testing documents belong to the release, always; design documents to the issue. |
| [tracking](#295-tracking-a-change-to-production) | `Refs #N` in the commit — **except in the merge lane, where `Closes #N` is correct**, because nothing else will ever close it. |
