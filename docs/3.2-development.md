...
It self-registers `e2e-*` players and purges them on teardown (`scripts/e2e-clean.sh`). Full details in [`e2e/README.md`](../e2e/README.md); in the release flow CI runs it as a gate — see [3.3](3.3-testing-ci-and-release.md#shipping-a-change-the-full-sequence) step 3, which is also where running it locally fits as the faster inner loop.
