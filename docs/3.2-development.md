...
It self-registers `e2e-*` players and purges them on teardown (`scripts/e2e-clean.sh`). Full details in [`e2e/README.md`](../e2e/README.md); in the release flow CI owns it as the regression pass — see [3.3](3.3-testing-ci-and-release.md#shipping-a-change-the-full-sequence) step 1.e. Running it locally is a faster inner loop, not a release gate.
