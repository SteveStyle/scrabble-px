...
```bash
./scripts/restore-prod-snapshot.sh                    # list what's on the VM
./scripts/restore-prod-snapshot.sh pre-0.4.13-...tgz  # restore one
```

The restore discards everything written since the snapshot, so it asks you to retype the name. It is a *rollback* mechanism, not a backup regime — the retention is days at most, and nothing copies these off the VM. For anything you'd be sorry to lose, take a real backup with the command above and keep it somewhere else.
