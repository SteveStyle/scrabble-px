...
**Preview** is the same command against its own volume, run locally rather than
over ssh — the volume name is the only difference:
...
Do not reach for the volume's host path (`/var/lib/docker/volumes/...`): it is
root-owned, so it needs `sudo`, and the throwaway container needs neither that
nor a copy. If you do copy the database out to read it with the host's own
`sqlite3`, take the `-wal` and `-shm` files with it — Preview's WAL routinely
holds more than a megabyte that the `.sqlite3` file alone does not yet contain,
so copying one file gives a silently stale answer.
