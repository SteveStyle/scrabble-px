...
| **PAR expiry** | *recorded here when the delivery creates it.* Expiry has no maximum, so the date is deliberate rather than as far out as the console allows |

**Restoring is asymmetric by construction.** The VM cannot read what it wrote, so a restore runs from a laptop or the console with real credentials — `scripts/restore-backup.sh`. That is a feature rather than an inconvenience: no credential on the production host can be used to exfiltrate the database.

**Two alarms watch it**, and both fire on *absence of success* rather than on a failure message — which also catches a crashed script, a timer nobody enabled, a host that is down, or a credential that expired:

| alarm | fires when |
| --- | --- |
| backup did not arrive | no new object for a day |
| no verified restore recently | the newest restore marker is over 100 days old — which is what turns *"we should test the restore quarterly"* into something that happens |
