...
**And it is an ongoing process, not a sequence of projects.** Owner, 2026-08-28:
*"Capacity Planning is an ongoing process."* Every workstream is unbounded, but
most produce discrete work: something is wrong, a project fixes it, the project
ends. This one runs a loop — measure, compare against the threshold, extrapolate,
raise what the extrapolation demands — and the loop never finishes.

**So its projects build or improve the loop rather than complete it.** A project
here establishes the series, writes the plan, or sets the review going; the
plan's own review is then an obligation that outlives the project that created
it. A capacity project that closes having *done capacity planning* has
