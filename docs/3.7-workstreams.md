...
| Capacity Planning | `crates/server-game/src/app/` — `sweeps.rs`, `throttle.rs` |
...
| `sweeps.rs` — the **mechanism**, and `expire_overdue_turns`, `expire_overdue_turn`, `send_move_time_reminders` | sweeps that apply game rules, and the thing that runs them |
...
| `sweeps.rs` — `expire_old_terminal_games`, `record_database_size` | what is removed, and when, and the daily size series |
