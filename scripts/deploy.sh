...
post_deploy() {
  local what="$1"; shift
  if ! "$@"; then
    POST_DEPLOY_FAILED+=("$what")
    echo "    warning: $what failed — production is live on $DEPLOYED_VERSION, continuing" >&2
    return 1
  fi
}
...
if (( IS_RELEASE )); then
  ...
  post_deploy "publishing the release notes" publish_release "$DEPLOY_TAG" "$DEPLOYED_VERSION" || true
fi
...
if (( ! IS_RELEASE )); then
  ...
  if post_deploy "the version bump commit" \
    ...
    post_deploy "the version bump push" \
...
post_deploy "settling the milestone" settle_milestone || true
...
post_deploy "the release-check announcement" announce_release_checks "$IS_RELEASE" "$DEPLOY_ENV" || true
...
if (( ${#POST_DEPLOY_FAILED[@]} > 0 )); then
  ...
  echo "    The deploy succeeded. These are the steps that record it, and they need doing by hand." >&2
  exit 3
fi
